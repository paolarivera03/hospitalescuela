...
@section('title','Panel Principal - Hospital Escuela')
...
<div class="p-4 p-md-5">
    ...
    <div class="welcome-card">
        ...
        <h2>Bienvenido, {{ $usuario['nombre'] ?? $usuario['usuario'] ?? 'Usuario' }} {{ $usuario['apellido'] ?? '' }}</h2>
        <div class="welcome-sub">{{ now()->locale('es')->isoFormat('dddd, D [de] MMMM [de] YYYY') }}</div>
    </div>

    @if($isAdminOrJefeDashboard)
...
<script>
(function () {
    const canSeeInventarioCards = @json($isAdminOrJefeDashboard);
    ...
    function renderPacientes(pacientes) {
        ...
        contenedor.innerHTML = pacientes.map(item => {
            ...
            const linkDetalle = idReaccion > 0
                ? `{{ url('/reacciones-adversas') }}/${idReaccion}`
                : '#';
    ...
    fetch('{{ route('ajax.reacciones_estadisticas_panel') }}', { credentials: 'same-origin' })
    ...
    if (canSeeInventarioCards) {
        fetch('{{ route('inventario.datos') }}', { credentials: 'same-origin' })
            .then(r => r.ok ? r.json() : null)
            .then(json => {
