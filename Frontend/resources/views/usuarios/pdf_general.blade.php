...
<html>
<head>
    <title>Catálogo de Usuarios – Hospital Escuela</title>
    <style>
        @page { margin: 0cm 0cm; }
...
<body>
    ...
    <div class="sub-header">Catálogo de Usuarios del Sistema</div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                {{-- Usamos ?? 'N/A' para evitar que el reporte falle si falta un dato --}}
                <td style="font-weight: bold;">{{ $u['usuario'] ?? 'N/A' }}</td>
                <td>{{ ($u['nombre'] ?? '') . ' ' . ($u['apellido'] ?? '') }}</td>
                <td style="text-transform: lowercase;">{{ $u['correo'] ?? 'N/A' }}</td>
                {{-- Aquí estaba el error: intentamos leer 'rol' o 'rol_nombre' --}}
                <td>{{ $u['rol'] ?? $u['rol_nombre'] ?? 'SIN ROL' }}</td>
                <td style="text-align: center;">{{ $u['estado'] ?? 'INACTIVO' }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
    ...
    <script type="text/php">
        if ( isset($pdf) ) {
            $pdf->page_script('
                ...
                // Posición para formato horizontal (Landscape): 720 es derecha, 555 es abajo
