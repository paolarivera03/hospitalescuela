...
.qa-block .wrap-input100:last-child{
margin-bottom:6px;
}
...
<form id="security-questions-form" class="login100-form validate-form" method="POST" action="{{ route('seguridad.preguntas.store') }}">
@csrf
...
@php
    $requiredQuestions = max(1, (int) ($requiredQuestions ?? 3));
@endphp

<div class="alert alert-info rule-box">
    Debes configurar {{ $requiredQuestions }} preguntas de recuperación.
</div>
...
<div id="security-error" class="alert alert-danger" style="display:none;"></div>

@for($i = 1; $i <= $requiredQuestions; $i++)
    <div class="qa-block">
        <div class="section-title">Pregunta {{ $i }}</div>
        <div class="wrap-input100">
            <select class="input100 security-question" name="preguntas[]" id="pregunta{{ $i }}">
                <option value="">Selecciona la pregunta {{ $i }}</option>
                @foreach($questions as $q)
                    <option value="{{ $q->id_pregunta }}">{{ $q->pregunta }}</option>
                @endforeach
            </select>
        ...
        <div class="wrap-input100">
            <input class="input100 security-answer"
                   type="text"
                   name="respuestas[]"
                   id="respuesta{{ $i }}"
...
<div class="container-login100-form-btn">
    <button type="submit" class="login100-form-btn" id="security-submit">
        Guardar y continuar
    </button>
</div>
...
const submitButton = document.getElementById('security-submit');
...
document.querySelectorAll('.security-answer').forEach((node) => {
    blockKeyRepeat(node);
...
form.addEventListener('submit', (e) => {
    ...
    const questionSelects = Array.from(document.querySelectorAll('.security-question'));
    const answerInputs = Array.from(document.querySelectorAll('.security-answer'));
    const selectedQuestions = questionSelects.map((node) => node.value).filter((value) => value !== '');
    const hasEmptyQuestion = questionSelects.some((node) => !node.value);
    const hasEmptyAnswer = answerInputs.some((node) => !node.value.trim());
    ...
    if (new Set(selectedQuestions).size !== selectedQuestions.length) {
        e.preventDefault();
        showError('Debes seleccionar preguntas diferentes.');
        return;
    }

    for (const node of answerInputs) {
        ...
        if (value.length < minRespuesta || value.length > maxRespuesta) {
            e.preventDefault();
            showError(`Cada respuesta debe tener entre ${minRespuesta} y ${maxRespuesta} caracteres.`);
            return;
        }
