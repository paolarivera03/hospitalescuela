...
@section('title', 'Reacciones Adversas - Hospital Escuela')
@section('header', 'Reacciones Adversas')
...
<div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
        ...
        <div class="d-flex align-items-center gap-2 flex-wrap">
            <span class="badge rounded-pill text-bg-light fw-bold px-3 py-2">
                <i class="fa-solid fa-list me-2"></i> Total: {{ $total }}
            </span>
                ...
                @if($canGuardarRAM)
                    <a href="{{ route('reacciones_adversas.create') }}"
                       class="btn rounded-pill fw-bold text-white px-4 shadow-sm"
...
<div class="glass p-3 mb-3">
    {{-- Fila 1: Buscador + botones de filtro --}}
    ...
    {{-- Fila 2: Ver [ ] Registros | Más recientes — debajo del buscador --}}
    <div class="d-flex align-items-center gap-2 flex-wrap">
        ...
        <table class="table align-middle mb-0" id="tablaRAM">
            ...
            <tbody>
                @foreach($reacciones as $r)
                    ...
                    <tr class="ram-row"
                        data-search="{{ strtolower($id . ' ' . $pac . ' ' . $med . ' ' . $desc . ' ' . $est) }}"
                        data-estado="{{ strtoupper(str_replace('_', ' ', $est)) }}"
                        data-id="{{ (int) $id }}"
                        data-ini="{{ $iniSortable }}">
                        <td class="ps-4 fw-black" style="font-weight:900;">{{ $loop->iteration }}</td>
                        <td class="fw-bold">{{ $pac }}</td>
                        <td class="fw-bold">{{ $med }}</td>
                        <td class="fw-bold text-dark">{{ $desc }}</td>
                        <td class="text-center">
                            <span class="badge bg-danger rounded-pill px-3 py-2 fw-bold fs-6">{{ $totalMedsPaciente }}</span>
                        </td>
                        <td>{{ $iniFmt }}</td>
                        <td>{{ $finFmt }}</td>
                        <td>
                            <span class="badge bg-{{ $badge['bg'] }} rounded-pill px-3 py-2 fw-bold">
                                <i class="fa-solid {{ $badge['icon'] }} me-2"></i>{{ $badge['text'] }}
                            </span>
                        </td>
                        <td class="text-end pe-4">
                            <div class="d-flex justify-content-end gap-2">
                                <a href="{{ route('reacciones_adversas.show', $r['id_reaccion']) }}"
                                ...
                                @if($canActualizarRAM)
                                    <a href="{{ route('reacciones_adversas.edit', $r['id_reaccion']) }}"
                                ...
                                @endif
                                    ...
                                    <button type="button"
                                        ...
                                        title="Reporte"
                                        aria-label="Reporte"
                                        onclick="descargarPDFIndividualRAM({{ (int) $r['id_reaccion'] }})">
                                    <i class="fa-solid fa-file-pdf"></i>
                                    </button>

                                @if($canEliminarRAM)
                                    <form action="{{ route('reacciones_adversas.destroy', $r['id_reaccion']) }}"
                                          method="POST"
...
<script>
    const reaccionesData = @json($reacciones ?? []);
    const ramById = new Map((reaccionesData || []).map((item) => [Number(item.id_reaccion || 0), item]));
    const usuarioActualPDF = @json($usuario['usuario'] ?? $usuario['nombre'] ?? session('usuario_nombre') ?? 'USUARIO');
