...
<head>
    ...
    <title>@yield('title', 'Hospital Escuela')</title>
    ...
    <div class="sidebar d-flex flex-column p-3" id="sidebar">
        <div class="text-center mt-3 mb-4">
            <a href="{{ route('dashboard') }}" title="Ir al dashboard">
                <img src="{{ asset('login-assets/images/logo-circle.png') }}"
        ...
        <ul class="nav flex-column mb-auto gap-2 px-2">
            <li>
                <a href="{{ route('dashboard') }}" class="nav-link nav-item-custom {{ Request::is('dashboard') ? 'active' : '' }}">
            ...
            <li>
                <a href="{{ route('inventario') }}"
                   class="nav-link nav-item-custom {{ Request::is('inventario*') ? 'active' : '' }}">
            ...
            <li>
                ...
                class="nav-link nav-item-custom {{ Request::is('reacciones-adversas*') ? 'active' : '' }}">
            ...
            @if($showSecurity)
            <li>
                <a class="nav-link nav-item-custom d-flex justify-content-between align-items-center {{ $isSecuritySection ? 'active' : '' }}"
                   data-bs-toggle="collapse"
                   href="#menu-seguridad"
                   role="button"
                   aria-expanded="{{ $isSecuritySection ? 'true' : 'false' }}"
                   aria-controls="menu-seguridad">
                    <span><i class="fas fa-shield-halved icon-sidebar"></i> Seguridad</span>
                    <i class="fas fa-chevron-down small"></i>
                </a>

                <div class="collapse {{ $isSecuritySection ? 'show' : '' }}" id="menu-seguridad">
                    <div class="nav-submenu">
                        @if($canUsu)
                        <a href="{{ route('usuarios.lista') }}" class="nav-subitem {{ $isUsersManagementRoute ? 'active' : '' }}">
                            <i class="far fa-user me-2"></i>Gestión
                        </a>
                        @endif

                        @if($canPermisos)
                        <a href="{{ route('usuarios.permisos.index') }}" class="nav-subitem {{ $isUsersPermissionsRoute ? 'active' : '' }}">
                            <i class="fas fa-user-shield me-2"></i>Permisos
                        </a>
                        @endif

                        @if($canRoles)
                        <a href="{{ route('roles.index') }}" class="nav-subitem {{ $isRolesRoute ? 'active' : '' }}">
                            <i class="fas fa-users-gear me-2"></i>Roles
                        </a>
                        @endif

                        @if($canBackup)
                        <a href="{{ route('backup.index') }}" class="nav-subitem {{ Request::is('backup*') ? 'active' : '' }}">
                            <i class="fas fa-database me-2"></i>Backup BD
                        </a>
                        @endif

                        @if($canParam)
                        <a href="{{ route('parametros.index') }}" class="nav-subitem {{ request()->routeIs('parametros.*') ? 'active' : '' }}">
                            <i class="fas fa-sliders me-2"></i>Parámetros
                        </a>
                        @endif

                        @if($canConfigAccesos)
                        <a href="{{ route('configuraciones.accesos.index') }}" class="nav-subitem {{ request()->routeIs('configuraciones.accesos.*') ? 'active' : '' }}">
                            <i class="fas fa-sliders-h me-2"></i>Configuraciones
                        </a>
                        @endif

                        @if($canBit)
                        <a href="{{ route('bitacora') }}" class="nav-subitem {{ Request::is('bitacora') ? 'active' : '' }}">
            ...
            @if(!$canInv && !$canRea && !$canUsu && !$canBit)
            <li>
    ...
    <div class="main-content" id="main-content">
        <div class="top-header-ribbon d-flex justify-content-between align-items-center px-4 py-3 shadow-sm">
            <div class="d-flex align-items-center">
                ...
                <h5 class="fw-bold mb-0">{{ $customHeader !== '' ? $customHeader : $headerTitle }}</h5>
            </div>
...
<div id="farewell-toast" class="farewell-overlay" aria-live="polite" aria-atomic="true">
    <div class="farewell-card">
        <p class="farewell-title">Hasta pronto, {{ $rawDisplayName }}</p>
                ...
                <div class="tab-content">
                    <div class="tab-pane fade show active" id="pane-datos" role="tabpanel" aria-labelledby="tab-datos">
                        <div class="profile-card">
                            <form action="{{ route('perfil.actualizar') }}" method="POST" class="row g-3" data-unsaved-form="true">
                                @csrf
                                <div class="col-md-6">
                                    <label class="form-label fw-bold">Nombre</label>
                                    <input type="text" name="nombre" class="form-control" value="{{ old('nombre', $currentUser['nombre'] ?? '') }}" required>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label fw-bold">Apellido</label>
                                    <input type="text" name="apellido" class="form-control" value="{{ old('apellido', $currentUser['apellido'] ?? '') }}" required>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label fw-bold">Correo</label>
                                    <input type="email" name="correo" class="form-control" value="{{ old('correo', $currentUser['correo'] ?? '') }}" required>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label fw-bold">Telefono</label>
                                    <input type="text" name="telefono" class="form-control" value="{{ old('telefono', $currentUser['telefono'] ?? '') }}">
                                </div>
                    ...
                    <div class="tab-pane fade" id="pane-password" role="tabpanel" aria-labelledby="tab-password">
                        <div class="profile-card">
                            <form action="{{ route('perfil.cambiar-password') }}" method="POST" class="row g-3" data-unsaved-form="true">
                                @csrf
                                <div class="col-12">
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        @if(session('success') && !session('report_prompt'))
            Swal.fire({
                icon: 'success',
                title: 'Listo',
        ...
        @if(session('report_prompt'))
            (() => {
                const reportPrompt = @json(session('report_prompt'));
                const yesUrl = reportPrompt?.yes_url || null;
                const noUrl = reportPrompt?.no_url || @json(route('dashboard'));
        ...
        @if($canRea && $isFarmaceutico)
            const bandejaEndpoint = @json(route('ajax.bandeja_pacientes'));
